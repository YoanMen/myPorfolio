...
export default function ProjectForm({
  ...
  const onHandleSlug = (value) => {
    ...
    setErrors({ ...errors, slug: inputValidation.checkLength(value, 100) });
  ...
  const handleSave = async (event, updating = false) => {
    ...
    const technologiesError =
      informations.technologies.length === 0 ? true : null;
  ...
  return (
    <div className="font-eudoxus flex flex-col gap-4">
      ...
      <div>
        ...
        />
        {errors.content && <small className="text-red">{errors.content}</small>}
      </div>
      <ExternalLinks
